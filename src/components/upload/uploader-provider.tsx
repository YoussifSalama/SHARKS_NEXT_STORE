...
export function UploaderProvider<TOptions = unknown>({
  ...
  const removeFile = React.useCallback(
    (key: string) => {
      ...
      setCoverIndex((prevCoverIndex) => {
        const indexRemoved = fileStates.findIndex((f) => f.key === key);
        if (indexRemoved === -1) return prevCoverIndex;
        if (indexRemoved === prevCoverIndex) return 0;
        if (indexRemoved < prevCoverIndex) return prevCoverIndex - 1;
        return prevCoverIndex;
      });
    },
    [fileStates, onFileRemoved],
  );
  ...
  const setCoverFile = React.useCallback((key: string) => {
    setFileStates((prev) => {
      const index = prev.findIndex((f) => f.key === key);
      if (index === -1) return prev;
      const newStates = [...prev];
      const [selected] = newStates.splice(index, 1);
      newStates.unshift(selected);
      return newStates;
    });
    setCoverIndex(0);
  }, []);
