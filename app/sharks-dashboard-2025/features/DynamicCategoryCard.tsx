...
const updateCategoryValidationSchema = (hasOldImg: boolean) =>
    z.object({
...
interface DynamicCategoryCardProps {
    ...
    handleUpdate: (formData: CategoryCardData) => void | Promise<void>;
    loading?: boolean;
    progress?: number;
}
...
const DynamicCategoryCard = ({
    ...
    const { register, handleSubmit, formState: { errors, isSubmitting }, getValues, setValue, trigger } = useForm({
        resolver: zodResolver(updateCategoryValidationSchema(hasOldImg)),
        mode: "onChange",
        defaultValues: {
            title: data.title,
            slogan: data.slogan,
    ...
    const handleCategoryUpdate = async (formData: any) => {
        ...
        const updatedData: CategoryCardData = {
            id: data.id,
            img: imgValue,
